...
import { PALETTE, WORLD } from '../../src/world/WorldConfig.js';
import { fogUniforms } from '../../src/render/Atmosphere.js';
import { clamp01 } from '../../src/core/MathUtils.js';
...
import { SURFACE_VERT, SURFACE_FRAG } from '../../src/shaders/water_surface.js';
...
export function buildWaterSurface(world, debug = null) {
    ...
    const outChunks = [];
    let cullVOk = 0, cullStep = 0, cullNp = 0;
      ...
      for (let bx = 0; bx < chunks; bx++) {
        ...
        for (let cz = cz0; cz < cz1; cz++) {
          for (let cx = cx0; cx < cx1; cx++) {
            ...
            if (hi - lo > SURF_LEVEL_STEP) { cullStep++; continue; }
        ...
        outChunks.push({
          pos: new Float32Array(pos), shore: new Float32Array(shoreA),
    ...
    if (debug) Object.assign(debug, {
      G, VG, quadM, origin, S, R, mask, wet, hasW, drawn, level,
      vLevel, vShore, vSpan, vDepth, vOk, span,
      sdfAt: (x, z) => 0,
      cullVOk, cullStep, cullNp,
    });
    void clamp01;
