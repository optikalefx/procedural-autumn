...
const out = await page.evaluate(async (P) => {
  ...
  // Baseline arm: push a species' home sites off the edge of the world so they
  // can never wake. Same page, same seed, same drive — the only honest way to
  // ask "what did adding this species cost the census".
  if (P.EXCLUDE.length) {
    const S = wl.sites;
    for (let i = 0; i < S.n; i++) {
      if (P.EXCLUDE.includes(wl.keys[S.spec[i]])) { S.x[i] = 1e9; S.z[i] = 1e9; }
    }
  }
