...
//  answer:
//
//   · **Whole world, always.** 3072 m fits in 170 px at 18 m per pixel. A
...
const WATER_SHALLOW = [126, 178, 206];
const WATER_DEEP = [39, 84, 122];
...
export class MiniMap {
  constructor(root, world) {
    ...
    // Decorative: everything it says is also said by the compass and the world
    // itself, and a screen reader has no use for a raster.
    ...
    // The player. A stubby arrowhead rather than a dot with a stick: at 14 px
    // a stick is one aliased pixel line and reads as noise, whereas a solid
  ...
  /**
   * Rasterise the world into `this.off` once. Reads the CPU-side typed arrays
   * on WorldData directly (`height`, `water`, `riverMask`) rather than the GPU
   * textures, because this needs a box filter over each block and a readback
   * would cost more than the arithmetic.
   */
  _bake() {
    const w = this.world;
    const R = w.res, N = BAKE;
    const height = w.height, water = w.water, riverMask = w.riverMask;
    if (!height || !water) return;
    ...
    // Height is box-averaged, but water and river are taken as the *maximum*
    // over each block. A river eight metres wide is under a pixel at this
    // scale, and averaging it away is exactly how a minimap ends up with no
    // rivers on it.
    ...
    // Percentile clip rather than min/max: one 320 m spire would otherwise
    // squash the whole valley into the bottom third of the ramp, and the valley
    // is the part the player is driving in.
    const { lo, hi } = this._range(H);
    const span = Math.max(1e-3, hi - lo);
    const iv = CONTOUR_STEPS.find((s) => span / s <= TARGET_CONTOURS) ?? 500;
    this.contourInterval = iv;
    ...
    const cell = w.worldSize / N;      // metres per map pixel

    for (let py = 0; py < N; py++) {
      for (let px = 0; px < N; px++) {
        const o = py * N + px;
        const h = H[o];
        let [r, gr, b] = rampAt((h - lo) / span);
        ...
        const sh = 0.66 + 0.62 * lam;
        r *= sh; gr *= sh; b *= sh;

        // Contours. A band boundary crossed between this texel and the one
        // west or north of it is a line — cheap, crisp, and always one pixel.
        const band = Math.floor(h / iv);
        const isLine = band !== Math.floor(hxa / iv) || band !== Math.floor(hza / iv);
        if (isLine) {
          // Darken toward the pixel's own colour rather than toward a fixed
          // ink, so a contour stays visible on cream summits and on plum
          // hollows alike.
          r *= 0.58; gr *= 0.58; b *= 0.62;
        }

        // Water last: it overrides both the tint and the contour, because a
        // contour drawn across a lake is a lie about a flat surface.
        const lake = clamp01((WET[o] - 0.22) / 0.42);
        const river = clamp01((RIV[o] - 0.10) / 0.45);
        const wt = Math.max(lake, river * 0.94);
        if (wt > 0.002) {
          ...
          r += (wr - r) * wt; gr += (wg - gr) * wt; b += (wb - b) * wt;
        }

        const k = o * 4;
        d[k] = r; d[k + 1] = gr; d[k + 2] = b; d[k + 3] = 255;
      }
    }
    g.putImageData(img, 0, 0);

    // The river *mask* gives every pixel its colour but not its continuity: eroded
    // channels drop below the mask threshold for a texel here and there and the
    // raster breaks into dashes. The polylines are the same rivers as a curve,
    // so stroking them faintly on top welds the dashes back into one thread.
  ...
  /** 1st–99th percentile of the height field, via a coarse histogram. */
  _range(H) {
  ...
  _strokeRivers(g, N) {
    ...
    g.lineCap = 'round';
    g.lineJoin = 'round';
    g.strokeStyle = 'rgba(112,164,196,0.62)';
    g.lineWidth = Math.max(1, N / 460);
    g.beginPath();
  ...
  update(x, z, bearing) {
    ...
    const w = this.world;
    if (!w) return;
    const u = clamp01((x + w.half) / w.worldSize);
    const v = clamp01((z + w.half) / w.worldSize);
